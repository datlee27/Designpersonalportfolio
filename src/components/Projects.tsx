...
export function Projects() {
  ...
  if (isMobile) {
    return (
      <section id="projects" className="relative min-h-screen bg-ink py-20 px-6 overflow-hidden">
        {/* Mobile Background */}
        ...
        <div className="relative z-10 container mx-auto">
          <h2 className="text-5xl font-heading tracking-tighter leading-none mb-12 uppercase text-white">
            SELECTED<br /><span className="text-accent">PROJECTS</span>
          </h2>

          <div className="flex flex-col gap-12">
            {projects.map((project) => (
              <div key={project.id} className="group relative flex flex-col bg-white/10 backdrop-blur-2xl border border-white/20 p-6 rounded-sm shadow-2xl transition-all duration-500 hover:border-accent/60">
                <div className="relative h-64 w-full mb-6 overflow-hidden border border-white/30">
                  <ImageWithFallback
                    src={project.image}
                    alt={project.title}
                    width={800}
                  ...
                  />
                  <div className="absolute top-0 left-0 bg-accent text-white px-3 py-1 font-mono text-sm uppercase tracking-tighter z-20">
                    PRJ_{project.id}
                  </div>
                </div>

                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-2xl font-heading tracking-tight text-white uppercase leading-tight">
                    {project.title}
                  </h3>
                ...
                <p className="text-sm text-paper leading-relaxed mb-6 font-medium opacity-90">
                  {project.description}
                </p>

                <div className="flex flex-wrap gap-2">
                  {project.tags.map(tag => (
                    <span key={tag} className="px-2 py-0.5 border border-white/20 text-[10px] font-mono text-white/80 uppercase tracking-widest bg-white/5">
                      {tag}
                    </span>
                  ))}
                </div>
            ...
            <div key={project.id} className="w-screen h-screen flex-shrink-0 flex items-center justify-center p-12 md:p-24 relative z-10">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-16 lg:gap-32 items-center max-w-7xl mx-auto">
                ...
                <div className="relative group">
                  ...
                  <div className="relative border border-white/20 bg-white/10 backdrop-blur-2xl p-5 overflow-hidden shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] transition-transform duration-700 group-hover:scale-[1.03] group-hover:border-accent/50">
                    <ImageWithFallback
                      src={project.image}
                      alt={project.title}
                      width={1200}
